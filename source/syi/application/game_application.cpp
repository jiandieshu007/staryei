...
#include "application/syi_imgui.hpp"
...
void GameApplication::create( const syi::ApplicationConfiguration& configuration ) {
    ...
    // App specific create ////////////////////////////////////////////////
    renderer = service_manager->get<syi::gfx::Renderer>();

    syi::gfx::RendererCreation rc{ gpu, &syi::MemoryService::instance()->system_allocator };
    renderer->init( rc );

    // imgui backend
    imgui = service_manager->get<syi::ImGuiService>();
    imgui->init( renderer );
...
void GameApplication::destroy() {
    ...
    syi::MemoryService::instance()->shutdown();
}
...
bool GameApplication::main_loop() {
    ...
    while ( !window->requested_exit ) {
        ...
        if ( !window->minimized ) {
            // Draw debug UIs
            syi::MemoryService::instance()->imgui_draw();

            syi::gfx::CommandBuffer* gpu_commands = renderer->get_command_buffer( syi::gfx::QueueType::Graphics, true );
