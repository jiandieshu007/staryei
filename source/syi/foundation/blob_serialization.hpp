...
// Defines:
// syi_BLOB_WRITE         - use it in code that can write blueprints,
//                            like data compilers.
...
inline void BlobSerializer::allocate_and_set( RelativePointer<T>& data, void* source_data ) {
    ...
    if ( source_data ) {
        syi::memory_copy( destination_memory, source_data, sizeof( T ) );
    }
}
...
inline void BlobSerializer::allocate_and_set( RelativeArray<T>& data, u32 num_elements, void* source_data ) {
    ...
    if ( source_data ) {
        syi::memory_copy( destination_memory, source_data, sizeof( T ) * num_elements );
    }
}
